...
export const Playlist: FC<Props> = ({ title, thumbnail, playlistId }) => {
  const { openModal, setContent } = useContext(ModalContext);

  const insertEditPlaylist2Modal = useCallback(
    (e: React.MouseEvent<SVGElement, MouseEvent>) => {
      e.preventDefault();
      e.stopPropagation();
      setContent(<EditPlaylist playlistId={playlistId} />);
      openModal();
    },
    []
  );
  ...
  return (
    <Wrapper>
      ...
      <Inline>
        ...
        <MoreButton onClick={insertEditPlaylist2Modal} />
      </Inline>
    </Wrapper>
  );
};
